...
Status HttpServiceBE::start() {
    ...
    auto* data_cache_action = new DataCacheAction(_env);
    _ev_http_server->register_handler(HttpMethod::GET, "/api/data_cache/{action}", data_cache_action);
    _http_handlers.emplace_back(data_cache_action);
