...
Status HdfsScanner::open_random_access_file() {
    ...
    if (_compression_type == CompressionTypePB::NO_COMPRESSION) {
        ...
        if (_scanner_params.use_data_cache) {
            _cache_input_stream = std::make_shared<io::CacheInputStream>(input_stream, filename, file_size,
                                                                         _scanner_params.modification_time);
            _cache_input_stream->set_enable_populate_cache(_scanner_params.enable_populate_data_cache);
...
void HdfsScanner::update_counter() {
    ...
    if (_scanner_params.use_data_cache && _cache_input_stream) {
        const io::CacheInputStream::Stats& stats = _cache_input_stream->stats();
        COUNTER_UPDATE(profile->data_cache_read_counter, stats.read_cache_count);
        COUNTER_UPDATE(profile->data_cache_read_bytes, stats.read_cache_bytes);
        COUNTER_UPDATE(profile->data_cache_read_timer, stats.read_cache_ns);
        COUNTER_UPDATE(profile->data_cache_write_counter, stats.write_cache_count);
        COUNTER_UPDATE(profile->data_cache_write_bytes, stats.write_cache_bytes);
        COUNTER_UPDATE(profile->data_cache_write_timer, stats.write_cache_ns);
        COUNTER_UPDATE(profile->data_cache_write_fail_counter, stats.write_cache_fail_count);
        COUNTER_UPDATE(profile->data_cache_write_fail_bytes, stats.write_cache_fail_bytes);
    }
