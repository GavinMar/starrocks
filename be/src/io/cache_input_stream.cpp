...
CacheInputStream::CacheInputStream(std::shared_ptr<SharedBufferedInputStream> stream, const std::string& filename,
                                   size_t size)
...
Status CacheInputStream::_read_block(int64_t offset, int64_t size, char* out, bool can_zero_copy) {
    ...
    if (ret.ok()) {
        ...
        if (sb->buffer.capacity() > 0) {
            ...
            _stats.read_shared_buffer_bytes += size;
            _stats.read_shared_buffer_count += 1;
            return Status::OK();
        }
    }
