...
Status parse_conf_data_cache_paths(const std::string& config_path, std::vector<std::string>* paths) {
    ...
    if ((path_vec.size() != paths->size() && !config::ignore_broken_disk)) {
        LOG(WARNING) << "fail to parse data_cache_disk_path config. value=[" << config_path << "]";
        return Status::InvalidArgument("fail to parse data_cache_disk_path");
    }
    return Status::OK();
}
